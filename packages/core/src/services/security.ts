...
import { listPromptOptimizations, runPromptOptimization } from "./optimizer.js";
...
export async function getSecurityScan(id: string) {
  ...
  const optimizations = await listPromptOptimizations(id);
  return { ...scan, findings, optimizations, latestOptimization: optimizations[0] ?? null };
}

export async function runSecurityScan(input: { promptId: string; versionNumber: number; optimize?: boolean }) {
  ...
  try {
    ...
    const heuristicRiskScore = totalRisk / Math.max(findingCount, 1);
    let finalRiskScore = heuristicRiskScore;
    let finalPassed = allPassed;
    ...
    if (input.optimize ?? true) {
      const optimization = await runPromptOptimization({ scanId, actor: "security_scan" });
      finalRiskScore = Math.max(heuristicRiskScore, optimization.overallRiskScore);
      finalPassed = allPassed && optimization.overallRiskLevel !== "high" && optimization.overallRiskLevel !== "critical";
    }
    ...
    await logAudit({
      ...
      entityId: scanId,
      detail: `risk=${finalRiskScore.toFixed(2)} passed=${finalPassed}`,
    });

    if (finalPassed) {
      await setPromptVersionStatus(version.id, "security_checked", `scan=${scanId} risk=${finalRiskScore.toFixed(2)}`);
    }
    ...
    await logAudit({
      ...
      entityType: "security_scan",
      entityId: scanId,
      detail: error instanceof Error ? error.message : "Unknown security scan failure",
    });
    throw error;
  }
}
