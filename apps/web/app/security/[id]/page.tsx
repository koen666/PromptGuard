...
export default async function SecurityDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <>
      <TemplatePageWrap>
        ...
        <div className="grid gap-4 md:grid-cols-4">
          ...
          <div className="rounded-md border border-white/10 bg-white/[0.04] p-4">
            ...
            <div className="mt-2 text-2xl font-semibold text-white">{getStatusLabel(scan.status)}</div>
          </div>
          <div className="rounded-md border border-white/10 bg-white/[0.04] p-4">
            <div className="text-xs text-white/40">提供商</div>
        ...
        <Panel className="mt-4">
          <div className="flex flex-col justify-between gap-4 lg:flex-row lg:items-start">
            <div>
              <div className="flex flex-wrap items-center gap-2">
                <h3 className="text-lg font-semibold text-white">GPT 结构化评审与提示词优化</h3>
              ...
              <p className="mt-1 max-w-3xl text-sm text-white/45">
                根据安全发现分析可逆向路径、泄露概率和修复方案，生成可审阅的新提示词草案。
              </p>
            </div>
            <OptimizerActions
              scanId={scan.id}
          ...
          {!review ? (
            <p className="mt-5 rounded-md border border-white/10 bg-white/[0.03] p-4 text-sm text-white/45">
              还没有优化草案。点击生成后会调用评审模型输出结构化 JSON 和新版提示词。
            </p>
          ) : (
            <div className="mt-5 grid gap-4 xl:grid-cols-12">
              <div className="xl:col-span-4">
                <div className="grid gap-3 sm:grid-cols-3 xl:grid-cols-1">
                  <div className="rounded-md border border-white/10 bg-white/[0.03] p-3">
                    ...
                    <div className="mt-2 text-xl font-semibold text-white">{getStatusLabel(review.overallRiskLevel)}</div>
                  </div>
                  <div className="rounded-md border border-white/10 bg-white/[0.03] p-3">
                    <div className="text-xs text-white/40">风险分</div>
              ...
              <div className="space-y-4 xl:col-span-8">
                ...
                <div className="rounded-md border border-white/10 bg-[#0f1111]/80 p-4">
                  <h4 className="text-sm font-semibold text-white">优化后提示词草案</h4>
      ...
      <TemplateListSection title="风险发现" description="提示词正文静态扫描与诱导泄露测试，证据已脱敏。">
